...
def receive_fmf_metadata(name, path, object_list=False):
    ...
    items = [
        x for x in fmf_tree.climb() if x.name.endswith("/" + name) and "@" not in x.name
    ]
    if object_list:
        return items
    if len(items) == 1:
        ...
        raise Exception(
            "There is more FMF test metadata for item by name:{}({}) {}".format(
    ...
    def __init__(self):
        ...
        if not self.metadata:
            if not self.fmf_metadata_path:
                logger.info(
                    "setting self.fmf_metadata_path by class location."
                    " DO NOT use it in this way."
                    " Metadata are set in colin.core.loader (use proper path)"
                )
                self.fmf_metadata_path = os.path.dirname(
        ...
        try:
            ...
            logger.debug(
                "missing argument (%s) in FMF metadata key (%s): %s",
                error,
                self.metadata.name,
                self.metadata.data,
            )
